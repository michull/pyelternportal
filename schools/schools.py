...
with open(file=path_html, mode="w", encoding="utf-8") as fh:
    ...
    fh.write('<head>\n')
    fh.write('    <base target="_top">\n')
    fh.write('    <meta charset="utf-8">\n')
    fh.write('    <meta name="viewport" content="width=device-width, initial-scale=1">\n')
    fh.write('    <title>Schools</title>\n')
    fh.write(f'    <link rel="stylesheet" href="{LINK_CSS}" />\n')
    fh.write(f'    <script src="{LINK_JS}"></script>\n')
    fh.write('    <style>\n')
    fh.write('        html, body {\n')
    fh.write('            height: 100%;\n')
    fh.write('            margin: 0;\n')
    ...
    fh.write('</head>\n')
    fh.write('<body>\n')
    fh.write('    <div id="map" style="width: 100%; height: 100%;"></div>\n')
    fh.write('    <script>\n')
    fh.write("        const map = L.map('map').setView([51.163361, 10.447683], 7);\n")
    fh.write("        const tiles = L.tileLayer('" + LINK_TILE + "{z}/{x}/{y}.png', {\n")
    fh.write("            maxZoom: 19,\n")
    fh.write("            attribution: '&copy; OpenStreetMap'\n")
    fh.write("        }).addTo(map);\n")
    for s in data:
        if "geo" in s and s["geo"]:
            ...
            if "lat" in geo and geo["lat"] and "lon" in geo and geo["lon"]:
                lat = geo["lat"]
                lon = geo["lon"]
                fh.write(f"        const m_{s["school"]} = L.marker([{lat}, {lon}])\n")
                fh.write(f"            .bindTooltip('{s["school"]}: {s["name"]}').addTo(map);\n")
    fh.write('    </script>\n')
    fh.write('</body>\n')
    fh.write('</html>\n')
